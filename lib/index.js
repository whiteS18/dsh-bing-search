 ...
 *
 * The endpoint is fully configurable via the `web-search-bing` settings
...
function providerError(endpoint, message, cause) {
  return new WebError(
    `${message}\n\nThe web search request used endpoint ${JSON.stringify(endpoint)}. If that endpoint is not intended, guide the user to Settings → Plugins → Plugin configuration → web-search-bing, where they can change and save the endpoint, or set the endpoint key of the web-search-bing cordis config. Only the user should choose or change the endpoint.`,
    "WEB_PROVIDER_ERROR",
